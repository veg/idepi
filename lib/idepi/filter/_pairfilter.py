...
class PairFilter(BaseFilter):
    ...
    def __init__(self, alphabet=None, mincons=None, maxcons=None,
        ...
        if alphabet is None:
            alphabet = Alphabet()
        if maxcons is None:
            maxcons = PairFilter.__DEFAULT_MAX_CONSERVATION
        if mincons is None:
            mincons = PairFilter.__DEFAULT_MIN_CONSERVATION
        if maxgap is None:
            maxgap = PairFilter.__DEFAULT_MAX_GAP_RATIO
...
#         self.__run, self.__data, self.__colnames = False, None, None
    ...
    def __compute(alignment, alphabet, mincons, maxcons, maxgap,
        ...
        data = PairFilter._filter(alignment, alphabet, ign_idxs, loop_defs, refidx)
    ...
    def learn(self, alignment, refseq_offs):
        colnames, data, self.__aln_len, self.__ign_idxs = PairFilter.__compute(
    ...
    def filter(self, alignment):
        if self.__aln_len is None or self.__ign_idxs is None:
            raise RuntimeError('No PairFilter model computed')

        assert(alignment.get_alignment_length() == self.__aln_len)
        return PairFilter._filter(alignment, self.__alph, self.__ign_idxs, self.__loop_defs)
