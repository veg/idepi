...
#         self.__run, self.__data, self.__colnames = False, None, None
    ...
    @staticmethod
    def _filter(alignment, alphabet, ign_idxs, loop_defs):
        aln_len = alignment.get_alignment_length()
        stride = len(alphabet)
        ncol = aln_len * stride - len(ign_idxs)
        data = np.zeros((len(alignment), ncol + len(loop_defs)), dtype=int)
        ...
        for start, end in (v for _, v in loop_defs):
            ...
            for i, row in enumerate(hvr):
                data[i, k] = sum(1 for p in row if p != Alphabet.SPACE)
            k += 1
